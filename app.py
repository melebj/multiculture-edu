...
def generate_content(
    ...
    # 런타임 의존성 import: 패키지 미설치 시 앱 전체 크래시를 방지하고 안내 메시지를 제공한다.
...
def main() -> None:
    ...
    if ask:
        ...
        try:
            ...
            st.error(
                "필수 패키지가 설치되지 않았습니다. `pip install -r requirements.txt`로 설치 후 다시 실행해 주세요."
            )
        except ValueError as err:
            st.error(f"입력값 오류: {err}")
        except Exception as err:  # API/네트워크/인증 등 오류를 포함한 안전망
            message = str(err)
            lower = message.lower()
            if "permissiondenied" in lower or "permission denied" in lower:
                st.error("API 키가 올바르지 않거나 권한이 없습니다. 키를 다시 확인해 주세요.")
            elif "unauthenticated" in lower or "api key" in lower:
                st.error("인증에 실패했습니다. API 키를 확인해 주세요.")
            elif "resourceexhausted" in lower or "quota" in lower:
                st.error("요청 한도를 초과했습니다. 잠시 후 다시 시도해 주세요.")
            else:
                st.error(f"Gemini 호출 중 오류가 발생했습니다. 잠시 후 다시 시도해 주세요. ({err})")
